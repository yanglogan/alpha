...
public abstract class AbstractAuthorityBridgeDAO implements AuthorityBridgeDAO {
	...
	public List<AuthorityBridgeLink> getAuthorityBridgeLinks() {
		...
		Collection<QName> typeQnames = dictionaryService.getSubTypes(ContentModel.TYPE_AUTHORITY_CONTAINER, true);
		typeQnames.add(ContentModel.TYPE_AUTHORITY_CONTAINER);
		
		List<AuthorityBridgeLink> res = new ArrayList<AuthorityBridgeLink>();
		for (QName typeQname : typeQnames) {
			
			Long authorityContainerTypeQNameId = Long.MIN_VALUE;
			Pair<Long, QName> authorityContainerTypeQNamePair = qnameDAO.getQName(typeQname);
			...
			Pair<Long, QName> authorityNameQNamePair =
				qnameDAO.getQName(ContentModel.PROP_AUTHORITY_NAME);
			...
			res.addAll(selectAuthorityBridgeLinks(authorityContainerTypeQNameId, memberAssocQNameId,
				authorityNameQNameId, storeId));
		}
	...
	public List<AuthorityBridgeLink> getDirectAuthoritiesForUser(NodeRef authRef) {
		...
		for (QName typeQname : typeQnames) {
			...
			res.addAll(selectDirectAuthoritiesForUser(authorityContainerTypeQNameId, memberAssocQNameId,
				authorityNameQNameId, storeId, (pair == null) ? -1L : pair.getFirst()));
		}
	 ...
	 * @return
	 */
	protected abstract List<AuthorityBridgeLink> selectDirectAuthoritiesForUser(
			Long authorityContainerTypeQNameId, Long memberAssocQNameId, Long authorityNameQNameId,
			Long storeId, Long nodeId);
	 ...
	 * @return
	 */
	protected abstract List<AuthorityBridgeLink> selectAuthorityBridgeLinks(
			Long authorityContainerTypeQNameId, Long memberAssocQNameId, Long authorityNameQNameId,
			Long storeId);
