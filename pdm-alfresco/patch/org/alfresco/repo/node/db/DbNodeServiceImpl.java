...
public class DbNodeServiceImpl extends AbstractNodeServiceImpl {
	...
	/**
	 * @param nodeIndexer the indexer that will be notified of node additions, modifications and
	 *        deletions
	 */
	...
	/**
	 * Set whether <b>cm:auditable</b> timestamps should be propagated to parent nodes where the
	 * parent-child relationship has been marked using <b>propagateTimestamps<b/>.
	 * 
	 * @param enableTimestampPropagation <tt>true</tt> to propagate timestamps to the parent node
	 *        where appropriate
	 */
	 ...
	 * and related properties. Existing values will not be overridden. All required pre- and
	 * post-update notifications are sent for missing aspects.
	 * 
	 * @param nodePair the node to which the details apply
	 * @param classQName the type or aspect QName for which the defaults must be applied. If this is
	 *        <tt>null</tt> then properties and aspects are only applied for 'extra' aspects and
	 *        'extra' properties.
	 ...
	 * @param extraAspects any aspects that should be added to the 'missing' set (may be
	 *        <tt>null</tt>)
	 * @param extraProperties any properties that should be added the the 'missing' set (may be
	 *        <tt>null</tt>)
	 * @param overwriteExistingProperties <tt>true</tt> if the extra properties must completely
	...
	/**
	 * Get any aspects that should be added given the type, properties and existing aspects. Note
	 * that this <b>does not</b> included a search for properties required for the missing aspects.
	 * 
	...
	private Set<QName> getMissingAspects(Set<QName> existingAspects, Map<QName, Serializable> existingProperties, QName classQName) {
		...
		for (QName missingAspect : missingAspectsCopy) {
			Set<QName> furtherMissingAspects = getMissingAspects(allTypesAndAspects, Collections.<QName, Serializable>emptyMap(), missingAspect);
	...
	public void setType(NodeRef nodeRef, QName typeQName) throws InvalidNodeRefException {
		...
		// Add the default aspects and properties required for the given type. Existing values will
		// not be overridden.
	...
	public void removeAspect(NodeRef nodeRef, QName aspectTypeQName) throws InvalidNodeRefException, InvalidAspectException {
		...
		if (aspectDef != null) {
			...
			// We have to iterate over the associations and remove all those between the parent and
			// child
			...
			NodeDAO.ChildAssocRefQueryCallback callback = new NodeDAO.ChildAssocRefQueryCallback() {
				...
				public boolean handle(Pair<Long, ChildAssociationRef> childAssocPair, Pair<Long, NodeRef> parentNodePair, Pair<Long, NodeRef> childNodePair) {
					...
					// Double check that it's not a primary association. If so, we can't delete it
					// and
	...
	private class DeleteFlag {
		public boolean canDelete = true;
	}
	
	private void checkDelete(DeleteFlag df, NodeRef nodeRef) {

		if (!df.canDelete) {
			return;
		}
		
		if (permissionService.hasPermission(nodeRef, PermissionService.DELETE_NODE) != AccessStatus.ALLOWED) {
			df.canDelete = false;
			return;
		}
		
		QName type = getType(nodeRef);
		if (ContentModel.TYPE_FOLDER == type || dictionaryService.isSubClass(type, ContentModel.TYPE_FOLDER)) {
			for (ChildAssociationRef rel : getChildAssocs(nodeRef, ContentModel.ASSOC_CONTAINS, RegexQNamePattern.MATCH_ALL)) {

				if (!rel.isPrimary()) {
					continue;
				}
				
				checkDelete(df, rel.getChildRef());
			}
		}
	 ...
	 * @param allowArchival <tt>true</tt> if normal archival may occur or <tt>false</tt> if the node
	...
	private void deleteNode(NodeRef nodeRef, boolean allowArchival) {
		...
		//BUG FIX
		DeleteFlag df = new DeleteFlag();
		QName type = getType(nodeRef);
		if (ContentModel.TYPE_FOLDER == type || dictionaryService.isSubClass(type, ContentModel.TYPE_FOLDER)) {
			checkDelete(df, nodeRef);
		}
	...
	/**
	 * Gets the properties map, sets the value (null is allowed) and checks that the new set of
	 * properties is valid.
	 * 
	...
	public void setProperty(NodeRef nodeRef, QName qname, Serializable value) throws InvalidNodeRefException {
		...
		boolean changed = addAspectsAndProperties(nodePair, null, null, null, null, Collections.singletonMap(qname, value), false);
	...
	/**
	 * Ensures that all required properties are present on the node and copies the property values
	 * to the <code>Node</code>.
	 * <p>
	 * To remove a property, <b>remove it from the map</b> before calling this method. Null-valued
	 * properties are allowed.
	 * <p>
	 * If any of the values are null, a marker object is put in to mimic nulls. They will be turned
	 * back into a real nulls when the properties are requested again.
	 * 
	...
	/**
	 * Specific properties <b>not</b> supported by
	 * {@link #getChildAssocsByPropertyValue(NodeRef, QName, Serializable)}
	 */
	...
	private NodeRef restoreNodeImpl(NodeRef archivedNodeRef, NodeRef destinationParentNodeRef, QName assocTypeQName, QName assocQName) {
		...
		List<ChildAssociationRef> parentAssocsToRemove = getParentAssocs(archivedNodeRef, ContentModel.ASSOC_ARCHIVED_LINK, RegexQNamePattern.MATCH_ALL);
		...
		// Prepare the node for restoration: remove old aspects and properties; add new aspects and
		// properties
	...
	public ChildAssociationRef moveNode(NodeRef nodeToMoveRef, NodeRef newParentRef, QName assocTypeQName, QName assocQName) {
		...
		if (movingStore) {
			...
			for (ChildAssociationRef oldChildAssoc : childAssocs) {
				// Fire before delete policy. Before create policy needs the new parent ref to
				// exist, so will be fired later
			...
			// The Node changes NodeRefs, so this is really the deletion of the old node and
			// creation
			...
			for (ChildAssociationRef oldChildAssoc : childAssocs) {
				...
				// Fire node policies. This ensures that each node in the hierarchy gets a
				// notification fired.
	...
	private void findNodeChildrenToMove(Long nodeId, final StoreRef storeRef, final List<ChildAssociationRef> childAssocsToMove, final Map<NodeRef, Long> nodeIds) {
	...
	/**
	 * Ensures name uniqueness for the child and the child association. Note that nothing is done if
	 * the association type doesn't enforce name uniqueness.
	 * 
	...
	/**
	 * Propagate, if necessary, a <b>cm:modified</b> timestamp change to the parent of the given
	 * association. The parent node has to be <b>cm:auditable</b> and the association has to be
	 * marked for propagation as well.
	 * 
	...
	private class AuditableTransactionListener extends TransactionListenerAdapter {
		...
		private void processSingle(final Long parentNodeId, final Date modifiedDate, boolean useCurrentTxn) {
			...
			try {
				...
				if (logger.isDebugEnabled()) {
					logger.debug("Touched cm:modified date for node " + parentNodeId + " (" + modifiedDate + ")" + (useCurrentTxn ? " in txn " : " in new txn ") + nodeDAO.getCurrentTransactionId(false));
				}
			} catch (Throwable e) {
